...
let profile = new URLSearchParams(window.location.search)
let userId = profile.get('user')
...
function postsRequest() {
  loadingToggle(true)
  axios.get(url+'users/'+userId+'/posts')
    .then((response) => {
      ...
      posts.forEach((post) => {
        ...
        let postHtml = `
          <div class="container col-lg-7" ${checkedOwenPost ? 'id="' + post.id + '"' : ''}>
    ...
    .catch(() => {
      popUpAlert("posts request error", "danger");
    })
    .finally(() => {
      loadingToggle(false)
    })
...
function userInfo(){
  ...
  .then((response)=>{
    let img = "./pics/profile-pics/7309681.jpg"
    let user = response.data.data
    
    document.getElementById('profile-image').src = checkedOwenImage(user.profile_image)
    document.getElementById('profile-name').innerHTML = user.name
    document.getElementById('profile-user-name').innerHTML = user.username
